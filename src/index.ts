...
const [match, matchP, matchI, matchPI] =
	makeMatchers("type");

const TEParSequenceS = Ap.sequenceS(TE.ApplyPar);
const RTEParSequenceS = Ap.sequenceS(
	RTE.ApplyPar,
);
...
const getBot = pipe(
	RTE.asks<GetBotDeps, GetBotDeps["telegraf"]>(
		(x) => x.telegraf,
	),
	RTE.chainEitherK(({ options, botToken }) =>
		Telegraf.init(options)(botToken),
	),
);
...
type ToImageDeps = {
	html: { options: Html.ToImageOptions };
};
export const toImageRTE = pipe(
	RTE.asks<ToImageDeps, ToImageDeps["html"]>(
		(x) => x.html,
	),
...
function subscribe({
	...
	bot.on("text", async (ctx, next) => {
		...
		if (O.isSome(codeBlocks)) {
			...
			(
				await Promise.all(
					runAndReactOnCodeBlock.map((run) =>
						run(),
					),
				)
			).map(
...
const program = pipe(
	...
	RTE.bindW("toImageDeps", () =>
		RTE.ask<ToImageDeps>(),
	),
	RTE.chainFirstIOK(
		({ bootstrapResult, toImageDeps }) => {
			// toImageDeps ??????? WTF
			return subscribeR({
